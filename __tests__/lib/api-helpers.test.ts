...
describe('requireAuth helper', () => {
    it('returns 401 when there is no session', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({ error: 'No autenticado. Inicia sesión para continuar.' });
    });
    ...
    it('returns 403 when user does not have required role', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({ error: 'Acceso denegado. No tienes permisos suficientes.' });
    });
});
