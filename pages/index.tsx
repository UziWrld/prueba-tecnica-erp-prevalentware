      ...
      <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
        {cards.map((card, i) => {
          ...
          return (
            <Card
              key={i}
              className='hover:shadow-lg transition-all hover:-translate-y-1 duration-200 border-none shadow-md overflow-hidden bg-white group'
            >
              <CardHeader className='pb-4'>
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center ${card.bg} mb-4 group-hover:scale-110 transition-transform`}
                >
              ...
              <CardContent className='pt-0'>
                <Link href={card.href} className='w-full'>
                  <Button
                    variant='outline'
                    className='w-full justify-between items-center group-hover:bg-slate-50 transition-colors'
                  >
