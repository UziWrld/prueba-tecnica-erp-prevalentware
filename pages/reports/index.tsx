...
import { LayoutDashboard, Download, Wallet, ArrowUpRight, ArrowDownRight, RefreshCw, ShieldAlert } from 'lucide-react';
...
export default function Reports({ user }: { user: any }) {
    ...
    const chartData = data?.movements.reduce((acc: any[], current: any) => {
        // Formatear la fecha (ej: 'ene. 2024')
        const month = new Date(current.date).toLocaleString('es-CO', { month: 'short', year: 'numeric' });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <Button onClick={handleDownloadCSV} disabled={!data || loading} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-md shadow-emerald-500/20">
            ...
            {loading ? (
                ...
                <div className="grid gap-6 md:grid-cols-3">
                            ...
                            <div className="mt-6 space-y-3 border-t border-slate-700 pt-4">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-400 flex items-center gap-1">
                                        <ArrowUpRight className="w-4 h-4 text-emerald-400" /> Total Ingresos
                                    </span>
                                    <span className="font-medium text-emerald-400">
                                        + $ {data?.movements.filter((m: any) => m.type === 'INCOME').reduce((a: number, b: any) => a + b.amount, 0).toLocaleString('es-CO')}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-400 flex items-center gap-1">
                                        <ArrowDownRight className="w-4 h-4 text-rose-400" /> Total Egresos
                                    </span>
                                    <span className="font-medium text-rose-400">
                                        - $ {data?.movements.filter((m: any) => m.type === 'EXPENSE').reduce((a: number, b: any) => a + b.amount, 0).toLocaleString('es-CO')}
                                    </span>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <Card className="md:col-span-2 shadow-sm border-slate-200">
                        <CardHeader>
                            <CardTitle className="text-lg text-slate-800">Flujo de Caja Histórico</CardTitle>
                            <CardDescription>Comparativa mensual de Ingresos y Egresos registrados</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="h-[300px] w-full mt-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <Bar dataKey="Egresos" fill="#f43f5e" radius={[4, 4, 0, 0]} barSize={32} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </CardContent>
